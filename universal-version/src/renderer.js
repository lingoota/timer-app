document.addEventListener('DOMContentLoaded', function() {
    ...
    const dom = {
        ...
        welcomeSystemName: document.getElementById('welcome-system-name'),
        startFirstSessionBtn: document.getElementById('start-first-session'),
        // Google Sheets 設定相關
        sheetsSetupOverlay: document.getElementById('sheets-setup-overlay'),
        setupStepWelcome: document.getElementById('setup-step-welcome'),
        setupStepAuth: document.getElementById('setup-step-auth'),
        setupStepSheets: document.getElementById('setup-step-sheets'),
        setupStepComplete: document.getElementById('setup-step-complete'),
        startSheetsSetupBtn: document.getElementById('start-sheets-setup'),
        skipSheetsSetupBtn: document.getElementById('skip-sheets-setup'),
        startGoogleAuthBtn: document.getElementById('start-google-auth'),
        authStatusDiv: document.getElementById('auth-status'),
        createNewSheetBtn: document.getElementById('create-new-sheet'),
        selectExistingSheetBtn: document.getElementById('select-existing-sheet'),
        sheetSelector: document.getElementById('sheet-selector'),
        sheetUrlInput: document.getElementById('sheet-url-input'),
        sheetsActions: document.getElementById('sheets-actions'),
        testSheetConnectionBtn: document.getElementById('test-sheet-connection'),
        confirmSheetSetupBtn: document.getElementById('confirm-sheet-setup'),
        sheetStatusDiv: document.getElementById('sheet-status'),
        setupSummary: document.getElementById('setup-summary'),
        connectionStatus: document.getElementById('connection-status'),
        sheetName: document.getElementById('sheet-name'),
        lastSync: document.getElementById('last-sync'),
        completeSheetsSetupBtn: document.getElementById('complete-sheets-setup'),
        // 用戶設定相關
        ...
        settingsSystemName: document.getElementById('settings-system-name'),
    ...
    const state = {
        ...
        isFirstTime: false,
        // Google Sheets 相關狀態
        sheetsSetupStep: 'welcome', // welcome, auth, sheets, complete
    ...
    // 初始化
    console.log('🚀 時間追蹤器 Universal v2.0.0 初始化開始...');
    ...
    console.log('welcomeOverlay:', !!dom.welcomeOverlay);
    ...
    // 延遲執行 Windows 用戶初始化，確保DOM完全載入
    setTimeout(() => {
        console.log('開始初始化Windows用戶系統...');
        initializeWindowsUser();
    }, 100);
    ...
    async function saveRecordToSheets(user, category, duration, startTime, endTime, sessionCount) {
        ...
        if (!state.sheetsConfig.useGoogleSheets) {
            console.log('未啟用Google Sheets同步，跳過');
            return;
        }
        ...
        try {
            ...
            const recordData = {
                userName: user,
                category: category,
                duration: Math.round(duration / 60), // 轉換為分鐘
                startTime: startTime.toISOString(),
                endTime: endTime.toISOString(),
                sessionId: generateSessionId(),
                notes: `第${sessionCount}次會話`,
                deviceName: await getDeviceName()
            };
            
            console.log('同步時間記錄到Google Sheets:', recordData);
            
            // 調用主進程的Google Sheets同步API
            const result = await window.api.invoke('google-sheets-sync-record', recordData);
            
            if (result.success) {
                updateSyncStatus('已同步');
                showToast('✅ 使用記錄已同步到 Google Sheets');
                console.log('✅ 記錄同步成功');
                
                // 更新最後同步時間
                state.sheetsConfig.lastSync = new Date().toISOString();
                saveUserSheetsConfig();
            ...
            showToast('❌ 無法同步到雲端，已保存本地備份');
            console.error('❌ Google Sheets同步失敗:', error);
            
            // 如果是認證問題，提示用戶重新設定
            if (error.message.includes('認證') || error.message.includes('授權')) {
                showToast('Google 授權已過期，請重新設定連接', 'warning');
            }
        }
    }
    ...
    async function getDeviceName() {
        try {
            if (state.windowsUser && state.windowsUser.username) {
                return `${state.windowsUser.username}@${require('os').hostname()}`;
            }
    ...
    // 保存用戶Google Sheets配置
    function saveUserSheetsConfig() {
        try {
            const configKey = `sheets_config_${state.windowsUser?.username || 'default'}`;
            localStorage.setItem(configKey, JSON.stringify(state.sheetsConfig));
            console.log('✅ Google Sheets配置已保存');
        } catch (error) {
            console.error('❌ 保存Google Sheets配置失敗:', error);
        }
    }
    
    // 載入用戶Google Sheets配置
    function loadUserSheetsConfig() {
        try {
            const configKey = `sheets_config_${state.windowsUser?.username || 'default'}`;
            ...
            if (savedConfig) {
                ...
                console.log('✅ 已載入Google Sheets配置:', state.sheetsConfig);
                return true;
            }
            return false;
        } catch (error) {
            console.error('❌ 載入Google Sheets配置失敗:', error);
            return false;
        }
    }
    ...
    async function checkGoogleSheetsConnection() {
        if (!state.sheetsConfig.useGoogleSheets) {
            updateSyncStatus('本地存儲');
            return;
        }
        
        try {
            updateSyncStatus('檢查連接...', true);
            
            // 檢查Google認證狀態
            const authResult = await window.api.invoke('google-auth-check');
            
            if (!authResult.success || !authResult.isAuthenticated) {
                updateSyncStatus('需要重新授權');
                console.log('⚠️ Google認證已過期，需要重新授權');
                return;
            }
            
            // 測試Google Sheets連接
            const connectionResult = await window.api.invoke('google-sheets-test');
            
            if (connectionResult.success) {
                updateSyncStatus('已連接 Google Sheets');
                console.log('✅ Google Sheets連接正常');
            } else {
                updateSyncStatus('連接失敗');
                console.log('❌ Google Sheets連接失敗:', connectionResult.error);
            }
            
        } catch (error) {
            updateSyncStatus('連接錯誤');
            console.error('❌ 檢查Google Sheets連接失敗:', error);
        }
    }
    ...
    async function performInitialDataSync() {
        if (!state.sheetsConfig.useGoogleSheets) {
            console.log('未啟用Google Sheets，跳過數據同步');
            return;
        }
        
        try {
            console.log('🔄 執行首次數據同步到 Google Sheets');
            updateSyncStatus('正在同步歷史數據...', true);
            
            // 獲取本地存儲的所有數據
            const localData = getLocalBackupData();
            
            if (!localData || localData.length === 0) {
                console.log('沒有本地數據需要同步');
                updateSyncStatus('無數據需要同步');
                return;
            }
            
            // 轉換本地數據格式為Google Sheets格式
            const formattedData = localData.map(record => ({
                userName: record.user || state.windowsUser?.displayName || '未知用戶',
                category: record.category || '未分類',
                duration: Math.round((record.duration || 0) / 60), // 轉換為分鐘
                startTime: record.startTime || record.timestamp || new Date().toISOString(),
                endTime: record.endTime || new Date().toISOString(),
                sessionId: record.sessionId || generateSessionId(),
                notes: record.notes || '歷史數據同步',
                deviceName: state.windowsUser?.username || 'Unknown Device'
            }));
            
            console.log(`準備同步 ${formattedData.length} 筆本地記錄`);
            
            // 批量同步到Google Sheets
            const result = await window.api.invoke('google-sheets-batch-sync', formattedData);
            
            if (result.success) {
                updateSyncStatus(`已同步 ${result.syncedCount} 筆記錄`);
                showToast(`✅ 成功同步 ${result.syncedCount} 筆歷史記錄到 Google Sheets`, 'success');
                
                // 更新最後同步時間
                state.sheetsConfig.lastSync = new Date().toISOString();
                saveUserSheetsConfig();
                
                console.log(`✅ 成功同步 ${result.syncedCount} 筆記錄到 Google Sheets`);
            } else {
                throw new Error(result.error || '批量同步失敗');
            }
            
        } catch (error) {
            updateSyncStatus('數據同步失敗');
            showToast('❌ 歷史數據同步失敗，請稍後重試', 'error');
            console.error('❌ 執行首次數據同步失敗:', error);
        }
    }
    ...
    function getLocalBackupData() {
        try {
            const backupData = localStorage.getItem('timeTrackingBackup');
            if (backupData) {
                return JSON.parse(backupData);
            }
            return [];
        } catch (error) {
            console.error('❌ 讀取本地備份數據失敗:', error);
            return [];
        }
    }
    ...
    // 初始化用戶選擇
    // Windows 用戶自動初始化
    async function initializeWindowsUser() {
        console.log('=== Windows 用戶自動初始化 ===');
        
        try {
            // 從主進程獲取 Windows 用戶資訊
            const result = await window.api.invoke('get-windows-user');
            
            if (result.success && result.user) {
                state.windowsUser = result.user;
                console.log('📋 獲取到Windows用戶:', state.windowsUser);
                
                // 載入用戶的Google Sheets配置
                loadUserSheetsConfig();
                
                // 檢查是否為首次使用
                const userData = localStorage.getItem('universalUserData');
                ...
                if (!userData) {
                    ...
                    autoLoginUser(savedUser);
                    
                    // 檢查Google Sheets連接狀態
                    checkGoogleSheetsConnection();
                }
            } else {
                console.error('❌ 無法獲取Windows用戶資訊:', result);
                // 使用後備方案
                handleUserDetectionFailure();
            }
        } catch (error) {
            console.error('❌ Windows用戶初始化失敗:', error);
            handleUserDetectionFailure();
        }
    }
    ...
    function showWelcomeScreen() {
        console.log('顯示新用戶歡迎畫面');
        
        // 更新歡迎畫面的用戶資訊
        if (dom.welcomeUserName) {
            dom.welcomeUserName.textContent = state.windowsUser.displayName;
        }
        if (dom.welcomeSystemName) {
            dom.welcomeSystemName.textContent = `@${state.windowsUser.username}`;
        }
        ...
        // 綁定開始按鈕事件
        if (dom.startFirstSessionBtn) {
            dom.startFirstSessionBtn.addEventListener('click', handleFirstSessionStart);
        }
    }
    
    // 處理首次會話開始
    function handleFirstSessionStart() {
        console.log('🚀 開始首次會話');
        
        // 創建新用戶資料
        const newUser = {
            id: generateUserId(),
            displayName: state.windowsUser.displayName,
            systemUsername: state.windowsUser.username,
            homedir: state.windowsUser.homedir,
        ...
        // 儲存用戶資料
        localStorage.setItem('universalUserData', JSON.stringify(newUser));
        ...
        console.log('✅ 新用戶資料已建立:', newUser);
        
        // 顯示 Google Sheets 設定選項
        setTimeout(() => {
            showSheetsSetupWizard();
        }, 500);
    }
    
    // 自動登錄現有用戶
    function autoLoginUser(userData) {
        console.log('🔐 自動登錄用戶:', userData.displayName);
        ...
        console.log('✅ 用戶自動登錄完成');
    }
    
    // 用戶檢測失敗的處理
    function handleUserDetectionFailure() {
        ...
        const userData = localStorage.getItem('universalUserData');
        if (userData) {
            autoLoginUser(JSON.parse(userData));
        } else {
            showWelcomeScreen();
        }
    }
    ...
    function updateUserDisplay() {
        if (state.selectedUser && dom.currentUserName) {
            dom.currentUserName.textContent = state.selectedUser.displayName;
        }
        
        // 更新設定畫面的用戶資訊
        if (state.windowsUser) {
            if (dom.settingsUserName) {
                dom.settingsUserName.textContent = state.windowsUser.displayName;
            }
            if (dom.settingsSystemName) {
                dom.settingsSystemName.textContent = `@${state.windowsUser.username}`;
            }
        }
    }
    ...
    function showUserSettings() {
        console.log('顯示用戶設定界面');
        
        // 更新設定畫面的用戶資訊
        updateUserDisplay();
        
        // 顯示設定覆蓋層
        if (dom.userSelectionOverlay) {
            dom.userSelectionOverlay.classList.remove('hidden');
        }
        
        // 綁定關閉按鈕事件（如果還沒綁定）
        if (dom.closeSettingsBtn && !dom.closeSettingsBtn.hasEventListener) {
            dom.closeSettingsBtn.addEventListener('click', hideUserSettings);
            dom.closeSettingsBtn.hasEventListener = true;
        }
    }
    
    // 隱藏用戶設定
    function hideUserSettings() {
        console.log('隱藏用戶設定界面');
    ...
    // === Google Sheets 設定嚮導功能 ===

    // 顯示 Google Sheets 設定嚮導
    function showSheetsSetupWizard() {
        console.log('📊 顯示 Google Sheets 設定嚮導');
        
        // 重置嚮導狀態
        state.sheetsSetupStep = 'welcome';
        
        // 顯示嚮導覆蓋層
        if (dom.sheetsSetupOverlay) {
            dom.sheetsSetupOverlay.classList.remove('hidden');
        }
        
        // 顯示歡迎步驟
        showSetupStep('welcome');
        
        // 綁定嚮導事件
        bindSheetsSetupEvents();
    }

    // 綁定 Google Sheets 設定事件
    function bindSheetsSetupEvents() {
        console.log('🔗 綁定 Google Sheets 設定事件');
        
        // 開始設定按鈕
        if (dom.startSheetsSetupBtn && !dom.startSheetsSetupBtn.hasEventListener) {
            dom.startSheetsSetupBtn.addEventListener('click', () => {
                console.log('開始 Google Sheets 設定');
                showSetupStep('auth');
            });
            dom.startSheetsSetupBtn.hasEventListener = true;
        }
        
        // 跳過設定按鈕
        if (dom.skipSheetsSetupBtn && !dom.skipSheetsSetupBtn.hasEventListener) {
            dom.skipSheetsSetupBtn.addEventListener('click', () => {
                console.log('跳過 Google Sheets 設定，使用本地存儲');
                skipSheetsSetup();
            });
            dom.skipSheetsSetupBtn.hasEventListener = true;
        }
        
        // 開始 Google 授權按鈕
        if (dom.startGoogleAuthBtn && !dom.startGoogleAuthBtn.hasEventListener) {
            dom.startGoogleAuthBtn.addEventListener('click', () => {
                console.log('開始 Google 授權流程');
                startGoogleAuth();
            });
            dom.startGoogleAuthBtn.hasEventListener = true;
        }
        
        // 創建新 Sheets 按鈕
        if (dom.createNewSheetBtn && !dom.createNewSheetBtn.hasEventListener) {
            dom.createNewSheetBtn.addEventListener('click', () => {
                console.log('選擇創建新的 Google Sheets');
                selectCreateNewSheet();
            });
            dom.createNewSheetBtn.hasEventListener = true;
        }
        
        // 選擇現有 Sheets 按鈕
        if (dom.selectExistingSheetBtn && !dom.selectExistingSheetBtn.hasEventListener) {
            dom.selectExistingSheetBtn.addEventListener('click', () => {
                console.log('選擇使用現有的 Google Sheets');
                selectExistingSheet();
            });
            dom.selectExistingSheetBtn.hasEventListener = true;
        }
        
        // 測試連接按鈕
        if (dom.testSheetConnectionBtn && !dom.testSheetConnectionBtn.hasEventListener) {
            dom.testSheetConnectionBtn.addEventListener('click', () => {
                console.log('測試 Google Sheets 連接');
                testSheetConnection();
            });
            dom.testSheetConnectionBtn.hasEventListener = true;
        }
        
        // 確認設定按鈕
        if (dom.confirmSheetSetupBtn && !dom.confirmSheetSetupBtn.hasEventListener) {
            dom.confirmSheetSetupBtn.addEventListener('click', () => {
                console.log('確認 Google Sheets 設定');
                confirmSheetSetup();
            });
            dom.confirmSheetSetupBtn.hasEventListener = true;
        }
        
        // 完成設定按鈕
        if (dom.completeSheetsSetupBtn && !dom.completeSheetsSetupBtn.hasEventListener) {
            dom.completeSheetsSetupBtn.addEventListener('click', () => {
                console.log('完成 Google Sheets 設定');
                completeSheetsSetup();
            });
            dom.completeSheetsSetupBtn.hasEventListener = true;
        }
    }

    // 顯示特定設定步驟
    function showSetupStep(step) {
        console.log('📝 切換到設定步驟:', step);
        
        // 隱藏所有步驟
        const steps = ['welcome', 'auth', 'sheets', 'complete'];
        steps.forEach(stepName => {
            const stepElement = dom[`setupStep${stepName.charAt(0).toUpperCase() + stepName.slice(1)}`];
            if (stepElement) {
        ...
        // 顯示當前步驟
        const currentStepElement = dom[`setupStep${step.charAt(0).toUpperCase() + step.slice(1)}`];
        if (currentStepElement) {
            currentStepElement.classList.remove('hidden');
        }
        
        state.sheetsSetupStep = step;
    }

    // 跳過 Google Sheets 設定
    function skipSheetsSetup() {
        console.log('⏭️ 跳過 Google Sheets 設定');
        
        // 設定為使用本地存儲
        state.sheetsConfig.useGoogleSheets = false;
        saveUserSheetsConfig();
        
        // 隱藏嚮導
        hideSheetsSetupWizard();
        
        // 顯示確認訊息
        showToast('已設定使用本地存儲，您隨時可以在設定中連接 Google Sheets', 'success');
    }
    ...
    async function startGoogleAuth() {
        ...
        // 顯示授權中狀態
        if (dom.authStatusDiv) {
            dom.authStatusDiv.classList.remove('hidden');
            dom.authStatusDiv.textContent = '正在開啟 Google 授權頁面...';
            dom.authStatusDiv.style.color = '#666';
        }
        
        try {
            // 調用主進程的Google OAuth服務
            const result = await window.api.invoke('google-auth-start');
            
            if (result.success) {
                // 授權成功
                state.googleAuthToken = true; // 標記已授權
                
                if (dom.authStatusDiv) {
                    dom.authStatusDiv.textContent = '✅ Google 授權成功！';
                    dom.authStatusDiv.style.color = '#4CAF50';
                }
                
                // 2秒後進入下一步
                setTimeout(() => {
                    showSetupStep('sheets');
                }, 2000);
                
            } else {
                // 授權失敗
                throw new Error(result.error || '授權失敗');
            }
            
        } catch (error) {
            console.error('❌ Google授權失敗:', error);
            
            if (dom.authStatusDiv) {
                dom.authStatusDiv.textContent = `❌ 授權失敗: ${error.message}`;
                dom.authStatusDiv.style.color = '#f44336';
            }
            
            showToast('Google授權失敗，請稍後重試', 'error');
        }
    }

    // 選擇創建新的 Google Sheets
    function selectCreateNewSheet() {
        console.log('➕ 選擇創建新的 Google Sheets');
        
        // 更新按鈕選中狀態
        updateSheetsOptionSelection('create');
        
        // 隱藏手動輸入選擇器
        if (dom.sheetSelector) {
            dom.sheetSelector.classList.add('hidden');
        }
        
        // 顯示動作按鈕
        if (dom.sheetsActions) {
            dom.sheetsActions.classList.remove('hidden');
        }
        
        // 準備創建新Sheets的設定
        state.sheetsSetupMode = 'create';
        state.currentSheetName = `${state.windowsUser.displayName} 的時間追蹤記錄`;
        
        // 更新測試按鈕文字
        if (dom.testSheetConnectionBtn) {
            dom.testSheetConnectionBtn.textContent = '🆕 創建並測試 Sheets';
        }
    }

    // 選擇現有的 Google Sheets
    function selectExistingSheet() {
        console.log('📄 選擇使用現有的 Google Sheets');
        
        // 更新按鈕選中狀態
        updateSheetsOptionSelection('existing');
        
        // 顯示手動輸入選擇器
        if (dom.sheetSelector) {
            dom.sheetSelector.classList.remove('hidden');
        }
        ...
        // 設定為連接現有Sheets模式
        state.sheetsSetupMode = 'existing';
        
        // 更新測試按鈕文字
        if (dom.testSheetConnectionBtn) {
            dom.testSheetConnectionBtn.textContent = '🧪 測試連接';
        }
        
        // 綁定輸入框事件
        if (dom.sheetUrlInput && !dom.sheetUrlInput.hasEventBound) {
            dom.sheetUrlInput.addEventListener('input', () => {
                const url = dom.sheetUrlInput.value.trim();
                if (url) {
                    // 從 URL 提取 Sheet ID
                    const sheetId = extractSheetIdFromUrl(url);
                    if (sheetId) {
                        state.currentSheetUrl = url;
                        console.log('檢測到有效的Google Sheets URL:', sheetId);
                    } else {
                        console.log('無效的Google Sheets URL格式');
                    }
                }
            });
            dom.sheetUrlInput.hasEventBound = true;
        }
    }

    // 更新 Sheets 選項按鈕選中狀態
    function updateSheetsOptionSelection(selected) {
        // 移除所有選中狀態
        if (dom.createNewSheetBtn) {
            dom.createNewSheetBtn.classList.remove('selected');
        }
        if (dom.selectExistingSheetBtn) {
            dom.selectExistingSheetBtn.classList.remove('selected');
        }
        
        // 添加選中狀態
        if (selected === 'create' && dom.createNewSheetBtn) {
            dom.createNewSheetBtn.classList.add('selected');
        } else if (selected === 'existing' && dom.selectExistingSheetBtn) {
            dom.selectExistingSheetBtn.classList.add('selected');
        }
    }

    // 從 URL 提取 Google Sheets ID
    function extractSheetIdFromUrl(url) {
        // Google Sheets URL 格式：https://docs.google.com/spreadsheets/d/SHEET_ID/edit
    ...
    async function testSheetConnection() {
        ...
        // 顯示測試中狀態
        showSheetStatus('loading', '正在測試連接...');
        
        try {
            let result;
            
            if (state.sheetsSetupMode === 'create') {
                // 創建新的Google Sheets
                console.log('創建新的Google Sheets:', state.currentSheetName);
                result = await window.api.invoke('google-sheets-create', state.currentSheetName);
                
                if (result.success) {
                    state.currentSheetId = result.sheetId;
                    state.currentSheetName = result.sheetName;
                    state.currentSheetUrl = result.sheetUrl;
                    
                    showSheetStatus('success', `✅ 成功創建 Google Sheets: ${result.sheetName}`);
                    state.sheetsConnected = true;
                } else {
                    throw new Error(result.error || '創建Google Sheets失敗');
                }
                
            } else if (state.sheetsSetupMode === 'existing') {
                // 連接現有的Google Sheets
                const sheetUrl = dom.sheetUrlInput ? dom.sheetUrlInput.value.trim() : '';
                
                if (!sheetUrl) {
                    throw new Error('請輸入Google Sheets URL');
                }
                
                console.log('連接到現有Google Sheets:', sheetUrl);
                result = await window.api.invoke('google-sheets-connect', sheetUrl);
                ...
                if (result.success) {
                    ...
                    showSheetStatus('success', `✅ 成功連接到 Google Sheets: ${result.sheetName}`);
                    state.sheetsConnected = true;
                } else {
                    throw new Error(result.error || '連接Google Sheets失敗');
                }
            } else {
                throw new Error('請先選擇創建新Sheets或連接現有Sheets');
            }
            
        } catch (error) {
            console.error('❌ Google Sheets連接測試失敗:', error);
            showSheetStatus('error', `❌ 連接失敗: ${error.message}`);
    ...
    function showSheetStatus(type, message) {
        if (dom.sheetStatusDiv) {
            dom.sheetStatusDiv.classList.remove('hidden', 'success', 'error', 'loading');
            dom.sheetStatusDiv.classList.add(type);
            dom.sheetStatusDiv.textContent = message;
        }
    }

    // 確認 Sheets 設定
    function confirmSheetSetup() {
        console.log('✅ 確認 Google Sheets 設定');
        
        if (!state.sheetsConnected) {
            showSheetStatus('error', '請先測試連接並確保成功');
            return;
        }
        ...
        state.sheetsConfig = {
            ...
            authToken: state.googleAuthToken
        };
        ...
        // 進入完成步驟
        showSetupStep('complete');
    }

    // 更新設定摘要
    function updateSetupSummary() {
        if (dom.connectionStatus) {
            dom.connectionStatus.textContent = '已連接';
        }
        if (dom.sheetName) {
            dom.sheetName.textContent = state.currentSheetName || '時間追蹤記錄';
        }
        if (dom.lastSync) {
            dom.lastSync.textContent = '剛剛';
        }
    }

    // 完成 Google Sheets 設定
    function completeSheetsSetup() {
        console.log('🏁 完成 Google Sheets 設定');
        ...
        // 顯示成功訊息
        if (state.sheetsConfig.useGoogleSheets) {
            showToast('🎉 Google Sheets 設定完成！您的數據將自動同步到雲端', 'success');
    ...
    // 隱藏 Google Sheets 設定嚮導
    function hideSheetsSetupWizard() {
        if (dom.sheetsSetupOverlay) {
            dom.sheetsSetupOverlay.classList.add('hidden');
        }
    }

    // 保存用戶 Sheets 配置
    function saveUserSheetsConfig() {
        localStorage.setItem('userSheetsConfig', JSON.stringify(state.sheetsConfig));
        console.log('💾 用戶 Sheets 配置已保存:', state.sheetsConfig);
    }

    // 載入用戶 Sheets 配置
    function loadUserSheetsConfig() {
        const saved = localStorage.getItem('userSheetsConfig');
        if (saved) {
            state.sheetsConfig = JSON.parse(saved);
            console.log('📋 載入用戶 Sheets 配置:', state.sheetsConfig);
        }
    }
    ...
    function performInitialSync() {
        ...
        // 這裡會實現實際的數據同步邏輯
        // 目前先顯示模擬訊息
        showToast('正在同步數據到 Google Sheets...', 'info');
